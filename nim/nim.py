...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        best_action = 0
        for action in available_actions:
            if self.get_q_value(state, action) > best_value or best_action == 0:
                best_action = action
                best_value = self.get_q_value(state, action)
                
        if epsilon == True:
            choice = random.choices((available_actions, best_action),weights)
            
            if choice[0] == best_action:
                return best_action
            return tuple(random.sample(available_actions,1)[0])
